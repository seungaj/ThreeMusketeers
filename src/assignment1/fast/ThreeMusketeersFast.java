...
public class ThreeMusketeersFast {
    ...
    private void setTimeLimit(int timeLimit){
        if (musketeerAgent instanceof FastHumanAgentStrategy){
            ((FastHumanAgentStrategy) musketeerAgent).setTimeLimit(timeLimit);
        }
        if (guardAgent instanceof FastHumanAgentStrategy){
            ((FastHumanAgentStrategy) guardAgent).setTimeLimit(timeLimit);
        }
    }
    /**
     * Default constructor to load Starter board
     */
    public ThreeMusketeersFast() {
     ...
     */
    public ThreeMusketeersFast(String boardFilePath) {
    ...
    private void selectMode(GameMode mode) {
        switch (mode) {
            case Human -> {
                musketeerAgent = new FastHumanAgentStrategy(board);
                guardAgent = new FastHumanAgentStrategy(board);
            }
            case HumanRandom -> {
                ...
                musketeerAgent = side.equals("M") ? new FastHumanAgentStrategy(board) : new RandomAgent(board);
                
                guardAgent = side.equals("G") ? new FastHumanAgentStrategy(board) : new RandomAgent(board);
            }
            case HumanGreedy -> {
                String side = getSideInput();
                musketeerAgent = side.equals("M") ? new FastHumanAgentStrategy(board) : new GreedyAgent(board);
                guardAgent = side.equals("G") ? new FastHumanAgentStrategy(board) : new GreedyAgent(board);
            }
        }
    }
...
//        }
    ...
    private boolean isHumansPlaying() {
        return musketeerAgent instanceof FastHumanAgentStrategy && guardAgent instanceof FastHumanAgentStrategy;
    }
    ...
    public int getTimeLimit(){
        int time = -1;
        System.out.println("Please enter time limit of moves in seconds");
        while (time == -1){
            ...
            try {
                ...
                if (time < 5){
                    time = -1;
                    System.out.println("Please enter value of more than 5 seconds");
                }
    ...
    public static void main(String[] args) {
        ...
        ThreeMusketeersFast game = new ThreeMusketeersFast(boardFileName);
        if (game.login()) register.login();
        else register.register();
        int timeLimit = game.getTimeLimit();
        if (timeLimit != -1){
            game.timeLimit = timeLimit*1000;
        }
        game.play();
    }
}
